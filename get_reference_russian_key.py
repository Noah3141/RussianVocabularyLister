...
# Wards increase efficiency by doing early broad sorting, such that words are not
# Passed through russ_match an unnecessary amount of times. If a word ends in 
# a clear verb ending, it may enter verb based ending matches, if not, it has
# not need to run through multiple verb ending checks



ward_noun_endings = [ending for ending in ение_endings and ание_endings and ство_endings and тель_endings and ость_endings]
...
for word in word_list[8000:10000:2]:
    ...
    if any(word.endswith(ending) for ending in я_feminine_endings):
        ...
        if word[:-1] + "ем" not in word_list:
            if russ_match(word, я_feminine_endings) > 6: # If word seems to fit ение_endings above 3 forms, assume it's a -ение word...
                for ending in я_feminine_endings:
                    if word.endswith(ending):
                        stem = word[:-len(ending)]
                        break
                dict_form = stem + "я"
